...
@author: shuang
@goal:
    - analyze the relationship between the two-point slope and the actual residence time
    - plot scatter plots of Absorbed dose vs Organ and Residence Time vs Organ from all patients in the MySQL database
...
if __name__ == '__main__':
    """
        run the code by ResTime_Analysis.py [data_dir] [DB_auth_dir]
        [data_dir]: the directory to save the output files including .cvs and PDF plots
        [DB_auth_dir]: the directory of the MySQL authentication file
    """

    # Get the directories from USER INPUT
    data_dir = sys.argv[1]
    DB_auth_dir = sys.argv[2]

    # Get MySQL authenticiation info and establish MySQL connection to the database
    DB_usr = 'root'
    db_pw = ddb.get_DB_auth_info(DB_auth_dir,DB_usr)
    con = mdb.connect(host='127.0.0.1', user=DB_usr,passwd=db_pw, db='UCSFDoseDB')

    # Get he Residence time info from all patients
    qr_rt1 = "SELECT * FROM ResTimeInfo"
    df1 = query_mysql(con, qr_rt1)
    df1['OrganName_mod'] = df1['OrganName'].map(srcname_redef)

    # get the absorbed dose info from all patients
    ...
    # make Seaborn plot of Residence time vs Organ
    sns.set(font='Arial')
    ...
    plt.tight_layout()
    fig_name = '{}/ResTimeVSOrgans.pdf'.format(data_dir)
    ...
    g1.set_yticklabels(g1.get_yticklabels(), {'weight': 'bold', 'size': 13})
    plt.tight_layout()
    fig_name = '{}/AbsorbedDosVSOrgans.pdf'.format(data_dir)
    plt.savefig(fig_name)
    # plt.show()
    ...
    # Examine the relationship btw slope vs RT and make linear-fitted plots of Residence time vs. TAC slope
